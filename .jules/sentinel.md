...
## 2026-06-27 - [Remote Command Execution (RCE) via adb shell Argument Concatenation]
**Vulnerability:** The script previously passed arguments directly to `adb shell` using `adb -s "$SELECTED_DEVICE" shell "$@"`. Because `adb shell` internally concatenates all arguments and evaluates them in a subshell on the remote device, passing unsanitized input could lead to Remote Command Execution (RCE). An attacker could inject commands using shell metacharacters (e.g., `;`, `&`, `|`, `` ` ``) if any of the passed arguments contained untrusted data.
**Learning:** `adb shell` acts similarly to an unescaped `eval` command by concatenating and executing its arguments. Quoting variables before passing them to `adb shell` is not sufficient to prevent injection if the variables are evaluated again remotely.
**Prevention:** Always safely iterate through and escape each argument (e.g., using `printf "%q" "$arg"`) before passing them to `adb shell` or similar commands that re-evaluate arguments in a subshell.
