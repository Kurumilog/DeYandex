...
В Yandex Go/Taxi, как и в Навигаторе (`ru.yandex.yandexnavi`), часто возникает проблема постоянного опроса GPS после закрытия приложения. Вы можете применить `WAKE_LOCK ignore` для устранения расхода батареи. Будьте осторожны с `RUN_IN_BACKGROUND ignore`, если вы следите за поездкой в фоне.
